...
fromto <- fromto %>% 
  ...
  arrange(value)
fromto %>% group_by(destination) %>% reframe(x=sum(value))
fromto %>% filter(origin==destination) %>% arrange(desc(value)) %>% head()

# Figure
level_origin <- c("<1%","1-10%","10-20%","20-30%","30-40%","40-50%",
...
# order for box
(x_pos <- ratios %>% group_by(Region_EV,origin) %>% tally() %>% ungroup() %>% 
  group_by(Region_EV) %>% tally() %>% mutate(x=cumsum(n)))

box_x <- c(0,x_pos$x)+0.5
box_y <- c(0,cumsum(rev(x_pos$n)))+0.5
